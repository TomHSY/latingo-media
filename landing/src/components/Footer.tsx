...
function SocialIcon({ href, label, children }: { href: string; label: string; children: ReactNode }) {
  return (
    <a
      ...
      aria-label={label}
      data-event={`social_${label.toLowerCase()}`}
      className="w-10 h-10 rounded-full bg-surface flex items-center justify-center text-secondary-text hover:text-coral hover:bg-surface/80 transition-colors"
    >
      {children}
    </a>
  )
